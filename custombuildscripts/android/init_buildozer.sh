...
function PYTHON311_Install
{
  # IF we want to update to to python 3.9

  printf  "%s" "[${yel} Info ${end}] Installing (apt) Python3.11 ....."
  printf  "%s" "[ Info ] Installing (apt) Python3.11 ......" >> "$LOGFILE"
  ...
  APT_INSTALL_SILENT "python3.11"
  printf  "%s" "[ Info ] update-alternatives --install /usr/bin/python3 python3 /usr/bin/python3.11......" >> "$LOGFILE"
  sudo update-alternatives --install /usr/bin/python3 python3 /usr/bin/python3.11 3 >> "$LOGFILE"
  APT_INSTALL_SILENT "python3.11-distutils"
  # todo:check python version
  printf  "\r%s\n" "[${grn}  OK  ${end}] Installed (apt) Python3.11          "
...
rm -f $LOGFILE
ECHO "Init Buildozer Version 1.1"
...
then
  ...
  # seems to be required for python 3.11
