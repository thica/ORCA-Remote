...
from ORCA.action.ActionType import cActionType
...
class cActions:
    ...
    def LoadActionsAppStart(self) -> None:
        ...
        Logger.debug ('Load AppStart Actions XmlFile')
        oET_Root:Element
        ...
        try:
            ...
            self.LoadActionsSub(oET_Root=oET_Root ,uSegmentTag='appstartactions',uListTag='appstartaction',dTargetDic=self.dActionsPageStart,uFileName=str(oFnActionFile))
            # just in case we have some sub commands (mainly in the fall back actions command set)
            self.LoadActionsSub(oET_Root=oET_Root ,uSegmentTag='actions',uListTag= 'action',dTargetDic= self.dActionsCommands,uFileName=str(oFnActionFile))

        except Exception as e:
            ShowErrorPopUp(uTitle='LoadActionsAppStart: Fatal Error',uMessage='TheScreen: Fatal Error:Load Appstart Action XmlFile (%s)' % str(oFnActionFile),bAbort=True,oException=e)

        aActions:List[cAction] = self.dActionsPageStart.get('earlyappstart')
        if aActions:
            Logger.debug ('TheScreen: Calling Early Application StartActions')
            Globals.oEvents.ExecuteActionsNewQueue(aActions=aActions,oParentWidget=None,uQueueName='earlyappstart')
    ...
    def LoadActionsSub(self,*,oET_Root:Element,uSegmentTag:Optional[str], uListTag:str,dTargetDic:Dict[str,List[cAction]],uFileName:Union[str,cFileName]='') -> None:
        """
            replaceoptions
            * appendtoexisting
        ...
        enReplaceOption:eReplaceOption
        ...
        try:
            ...
            if oET_SegmentsStart is not None:
                for oET_Includes in oET_SegmentsStart:
                    if oET_Includes.tag=='includes':
                        ...
                        uName           = GetXMLTextAttribute(oXMLNode=oET_Includes,uTag='name',            bMandatory=True, vDefault='')
                        enReplaceOption = eReplaceOption.FromString(uReplaceOption= GetXMLTextAttribute(oXMLNode=oET_Includes,uTag='replaceoption',   bMandatory=False,vDefault='appendtoexisting'))
                        uNewname        = GetXMLTextAttribute(oXMLNode=oET_Includes,uTag='newname',         bMandatory=False,vDefault='')
                        aActions        = []
                        bOldExist       = True
                        ...
                        if enReplaceOption==eReplaceOption.eAppendToExisting:
                            aActions = aActionsOrg

                        if enReplaceOption==eReplaceOption.eRenameExisting and bOldExist:
                            dTargetDic[uNewname] = aActionsOrg

                        if enReplaceOption==eReplaceOption.eRenameMeIfExist and bOldExist:
                            uName=uNewname
            ...
            ShowErrorPopUp(uTitle='LoadActionsSub: Fatal Error',uMessage='TheScreen: Fatal Error:Load Action XmlFile:',bAbort=True,oException=e)
    ...
    def Dump(self,*,uFilter:str) -> None:
        ...
        for uActionName in sorted(self.dActionsCommands):
            if uFilter == '' or uFilter in uActionName:
