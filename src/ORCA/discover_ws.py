...
URN = uuid.uuid3(uuid.NAMESPACE_DNS, 'python.org').urn

def CleanXML(byXML:bytes):
    uXML = ToUnicode(byXML)
    uXML = uXML.replace("<s:","<")
    uXML = uXML.replace("</s:","</")
    uXML = uXML.replace("<a:","<")
    uXML = uXML.replace("</a:","</")
...
def parseProbeMessage(dXML:Dict):
    ...
    env.setMessageId(ParseDictAny(vObj=dXML,    uSearchKey="MessageID", bSingleValue=True))
    env.setReplyTo(ParseDictAny(vObj=dXML,      uSearchKey="ReplyTo",   bSingleValue=True))
    env.setTo(ParseDictAny(vObj=dXML,           uSearchKey="To",        bSingleValue=True))

    typeNodes = ParseDictAny(vObj=dXML, uSearchKey= "Types")
    if len(typeNodes) > 0:
        env.getTypes().extend(getTypes(typeNodes[0]))

    scopeNodes = ParseDictAny(vObj=dXML, uSearchKey=  "Scopes")
    if len(scopeNodes) > 0:
        env.getScopes().extend(getScopes(scopeNodes[0]))

    return env

def parseMessage(dXML:Dict):
    env = SoapEnvelope()
    env.setAction(ParseDictAny(vObj=dXML,            uSearchKey="Action", bSingleValue=True))
    env.setMessageId(ParseDictAny(vObj=dXML,         uSearchKey="MessageID",         bSingleValue=True))
    env.setTo(ParseDictAny(vObj=dXML,                uSearchKey="To",                bSingleValue=True))
    env.setInstanceId(ParseDictAny(vObj=dXML,        uSearchKey="InstanceId",        bSingleValue=True))
    env.setSequenceId(ParseDictAny(vObj=dXML,        uSearchKey="SequenceId",        bSingleValue=True))
    env.setMessageNumber(ParseDictAny(vObj=dXML,     uSearchKey="MessageNumber",     bSingleValue=True))
    env.setRelatesTo(ParseDictAny(vObj=dXML,         uSearchKey="RelatesTo",         bSingleValue=True))
    env.setRelationshipType(ParseDictAny(vObj=dXML,  uSearchKey="RelationshipType",  bSingleValue=True))
    env.setEPR(ParseDictAny(vObj=dXML,               uSearchKey="Address",           bSingleValue=True))
    env.setTypes(ParseDictAny(vObj=dXML,             uSearchKey="Types",             bSingleValue=True))
    env.setScopes(ParseDictAny(vObj=dXML,            uSearchKey="Scopes",            bSingleValue=True))
...
def parseHelloMessage(dXML:Dict):
    env = SoapEnvelope()
...
def parseProbeMatchMessage(dXML):
    env = SoapEnvelope()
    ...
    aEpr    = ParseDictAny(vObj=dXML,     uSearchKey="Address")
    aTypes    = ParseDictAny(vObj=dXML,     uSearchKey="Types")
    aScopes    = ParseDictAny(vObj=dXML,     uSearchKey="Scopes")
    aXAddrs    = ParseDictAny(vObj=dXML,     uSearchKey="XAddrs")
    aMdv    = ParseDictAny(vObj=dXML,     uSearchKey="MetadataVersion")

    for i in range(len(aEpr)):
        env.getProbeResolveMatches().append(ProbeResolveMatch(aEpr[i], aTypes[i], aScopes[i], aXAddrs[i], aMdv[i]))
...
def parseEnvelope(data):
    ...
    if soapAction == ACTION_PROBE:
        return parseProbeMessage(dXML)
    elif soapAction == ACTION_PROBE_MATCH:
        return parseProbeMatchMessage(dXML)
        ...
        return parseHelloMessage(dXML)

def sendMessage(sock, addr, port, data):
    bData = ToBytes(data)
    sock.sendto(bData, (addr, port))
...
class MessageSenderThread(threading.Thread):
    ...
    def addMessage(self, env, addr, port, initialDelay=0):
        msg = Message(env, addr, port, self.__udpRepeat,  self.__udpMinDelay, self.__udpMaxDelay, self.__udpUpperDelay, initialDelay)
...
class WSDiscovery:
    ...
    def __removeRemoteService(self, epr):
        if epr in self.__remoteServices:
    ...
    def handleEnv(self, env, addr):
        if (env.getAction() == ACTION_PROBE_MATCH):
            ...
            if env.getEPR() in self.__localServices:
    ...
    def __sendResolveMatch(self, service, relatesTo, addr):
        ...
        env.setMessageId(URN)
        env.setInstanceId(str(service.getInstanceId()))
        env.setMessageNumber(str(service.getMessageNumber()))
        env.setRelatesTo(relatesTo)

        env.getProbeResolveMatches().append(ProbeResolveMatch(service.getEPR(), service.getTypes(), service.getScopes(), service.getXAddrs(), str(service.getMetadataVersion())))
    ...
    def __startThreads(self):
        ...
        self.__multicastSenderThread = MessageSenderThread(self.__sockMultiOut, midMap, MULTICAST_UDP_REPEAT, MULTICAST_UDP_MIN_DELAY,  MULTICAST_UDP_MAX_DELAY, MULTICAST_UDP_UPPER_DELAY)
        self.__multicastSenderThread.start()

        self.__unicastSenderThread = MessageSenderThread(self.__sockUniOut, midMap, UNICAST_UDP_REPEAT, UNICAST_UDP_MIN_DELAY, UNICAST_UDP_MAX_DELAY, UNICAST_UDP_UPPER_DELAY)
