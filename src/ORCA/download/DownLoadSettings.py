...
class cDownLoad_Settings(EventDispatcher):
    ...
    def ConfigDownLoadSettings(self,*,oSetting:KivySettings) -> KivySettings:
        ...
        self.bForce                 = False
        uSettingsJSONResources:str  = '[{"type": "title","title": "$lvar(694)" },\n'
        iNumber:int                 = 0
        uURL:str
        uSubRep:str
        ...
        for uURL in Globals.aRepositories:
            if not uURL=='':
                uSettingsJSONResources+= f'{{"type": "bool","title": "$lvar(695) {iNumber:d}","desc": "{uURL}","section": "ORCA","key": "repository_state{iNumber:d}"}},\n'
            iNumber+=1

        uSettingsJSONResources    +='{"type": "buttons","title": "$lvar(696)","desc": "$lvar(697)","section": "ORCA","key": "button_load_resources","buttons":[{"title":"$lvar(5016)","id":"button_load_resources"}]}]\n'
                    ...
                    if len(oEntry.oDescriptions.dDescriptions)>0:
                        ...
                        if uDescription is None:
                            for uKey in oEntry.oDescriptions.dDescriptions:
                                ...
                                break
                    uSettingsJSON  +=   '{"type": "buttons","title": "%s ","desc": "%s %s:%s","section": "ORCA","key": "button_add_resource","buttons":[{"title":"$lvar(5016)","id":"button:%s:%s"}]},' % (oEntry.uName,uDescription,'$lvar(673)',oEntry.uVersion,uSubRep[1] ,oEntry.uName)
                uSettingsJSON:str=uSettingsJSON[:-1]+ ']'
    ...
    def LoadRepositoryDirectory(self,*,bDoNotExecute:bool) -> List[cAction]:
        ...
        if len(oRepository.aRepEntries)==0:
            for i in range(Globals.iCntRepositories):
                uRepKey='repository_state'+str(i)
                uRepUrl=Globals.aRepositories[i]
                if not uRepUrl=='':
                    if Config_GetDefault_Str(oConfig=Globals.oOrcaConfigParser, uSection='ORCA', uOption=uRepKey, vDefaultValue='0')!= '0':
                        oRepository.Init()
    ...
    def On_ConfigChange(self,oSettings:Optional[KivySettings], oConfig, uSection:str, uKey:str, uValue:str) -> bool:
        ...
        elif uKey.startswith('repository_state'):
            pass
        else:
    ...
    def DownloadDownloadQueue(self) -> None:
        ...
        #Execute the Queue
        StopWait()
        oEvents.ExecuteActionsNewQueue(aActions=aActions,oParentWidget=None,uQueueName="downloadqueue")
    ...
    def CreateDownloadQueueForRepositoryItem(self,*,uRepType:str,uRepName:str) -> None:
        ...
        if not uRef in self.aQueue:
            ...
            for oEntry in oRepository.aRepEntries:
                if oEntry.uRepType == uRepType and oEntry.uName==uRepName:
                    aEntry.append(oEntry)
            ...
            if len(aEntry)>0:
                ...
                if (oEntry.iVersion > iInstalledVersion) or self.bForce:
                    ...
                    Logger.info(f'Skipping download of repository type:{uRepType} Resource:{uRepName}, Resource version [{oEntry.iVersion:d}] not newer than installed version [{iInstalledVersion:d}]')

            else:
                LogError(uMsg=f'can\'t find Resource in Resourcelist: [{uRepType}] {uRepName}')
                ShowErrorPopUp(uMessage=f'can\'t find Resource in Resourcelist: [{uRepType}] {uRepName}')

def GetInstalledVersion(*,uType:str,uName:str) -> int:
    """ Gets the installed version of a repository element """
    uKey:str    = f'{uType}:{uName}'
