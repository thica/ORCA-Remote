...
class cLoadOnlineResource(EventDispatcher):
    ...
    def LoadSingleFile(self,*,uRef:str,oProgressBar:cProgressBar) -> bool:
        ...
        self.oFnDest         = cFileName(self.oRef.dPars['Dest'])
        ...
        try:
            Logger.debug(f'LoadOnlineResource: Downloading [{uUrl}] to [{self.oFnDest}]')
            # noinspection PyUnresolvedReferences
            self.uUrl = urllib.parse.quote(uUrl, safe='%/:=&?~#+!$,;\'@()*[]')

            #self.uUrl         = urllib.quote(uUrl,safe="%:=&?~#+!$,;'@()*[]")
            #todo: fix orcaremote ssl problem and remove verify=false
            self.oWeb         = UrlRequest(self.uUrl, verify=False, on_success=self.OnSuccess,on_failure=self.OnFailure,on_error=self.OnError,on_progress=self.OnProgress, decode=False,file_path=str(self.oFnDest), debug=False)

            return True
        except Exception as e:
            LogError(uMsg=f'Can\'t load online resource LSF: {self.uUrl} to {self.oFnDest}', oException=e)
            self.bFinished  = True
            self.bOnError   = True
    ...
    def OnFailure(self,request,result) -> None:
        ...
        self.bOnError  = True
        self.oFnDest.Delete()
        LogError(uMsg=f'Can\'t load online resource OnFailure: {self.uUrl} to {self.oFnDest} [{result}]')
    ...
    def OnSuccess(self,request,result) -> None:
        ...
        if not self.oFnDest.Exists():
            Logger.error(f'Target File not found: {self.oFnDest}')
            self.bOnError = True
            ...
            try:
                if not self.bOnError:
                    if self.uTarget.startswith('.' or '..' in self.uTarget):
                        ...
                        oZipFile:cZipFile = cZipFile('').ImportFullPath(uFnFullName=str(self.oFnDest))
                        if oZipFile.IsZipFile():
                            ...
                            if oZipFile.Exists():
                                Logger.error(f'Failed to unzip: {oZipFile}')
                            else:
                                Logger.error(f'Failed to download zip: {oZipFile}')
                                #todo: handle unzipped files
                        oZipFile.Delete()
                Logger.debug(f'LoadOnlineResource: Finished download Resource  [{self.uType}][{self.uName}]')
                RegisterDownLoad(uType=self.uType,uName=self.uName,iVersion=ToInt(self.uVersion))
            except Exception as e:
                LogError(uMsg='Can\'t unpack resources:'+self.uUrl,oException=e)
