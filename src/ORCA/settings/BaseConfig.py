...
class cBaseConfig:
    ...
    def __init__(self, oObject:cBaseObject):
        ...
        self.uCurrentSection:str                        = ''
        self.uDefaultConfigName:str                     = 'DEFAULT'
        self.uType:str                                  = ''
        self.uWidgetName:str                            = ''
    ...
    def Init(self) -> None:
        ...
        self.oFnConfig = cFileName(self.oObject.oPathMyData) + 'config.ini'

        if not self.oFnConfig.Exists():
            self.oConfigParser.filename = str(self.oFnConfig)
        ...
        try:
            ...
            if len(self.oConfigParser._sections) == 0:
                if self.oFnConfig.Exists():
                    self.oObject.ShowDebug(uMsg=f'Reading Config File: {self.oFnConfig.string}')
                    self.oConfigParser.read(str(self.oFnConfig))
            else:
                if not self.oFnConfig.Exists():
                    self.oConfigParser.write()
        except Exception as e:
            self.oObject.ShowError(uMsg=f'Can\'t load config file: {self.oFnConfig}', uParConfigName='', oException=e)
    ...
    def CreateSection(self,*,uSectionName:str) -> None:
        ...
        if self.oObject.oPathMyData is not None:
            self.oObject.ShowDebug(uMsg=f'Adding new section [{uSectionName}]')
    ...
    def GetSettingParFromIni(self,*,uSectionName:str, uVarName:str) -> str:
        ...
        if uResult is None:
            uResult = ''
            self.oObject.ShowError(uMsg=f'Can\'t find setting: {uSectionName}:{uVarName}')
        else:
            self.oObject.ShowDebug(uMsg=f'Returning setting: {uResult} from {uSectionName}:{uVarName}')
        return uResult
    ...
    def ConfigureKivySettings(self, *,oKivySetting:KivySettings,uConfig:str='') -> KivySettings:
        ...
        self.GetSectionList()
        SetVar(uVarName='SettingSectionList', oVarValue=self.CreateSectionsJSONString())
        ...
        for uSection in self.aSections:
            if uConfig=='' or uConfig==uSection:
                ...
                SetVar(uVarName='ObjectConfigSection', oVarValue=uSection)
    ...
    def On_ConfigChange(self, oSettings:KivySettings, oConfig:KivyConfigParser, uSection:str, uKey:str, uValue:str):
        ...
        if uKey == 'configchangebuttons':
            self.uCurrentSection = uSection
            if uValue == 'button_add':
                SetVar(uVarName='SCRIPTINPUT', oVarValue='DEVICE_dummy')
                self.oInputKeyboard = ShowKeyBoard(uDestVar='SCRIPTINPUT', oFktNotify=self.On_InputAdd)
            if uValue == 'button_delete':
                ShowQuestionPopUp(uTitle='$lvar(5003)', uMessage='$lvar(5044)', fktYes=self.On_InputDel, uStringYes='$lvar(5001)', uStringNo='$lvar(5002)')
            if uValue == 'button_rename':
                SetVar(uVarName='SCRIPTINPUT', oVarValue=uSection)
                self.oInputKeyboard = ShowKeyBoard(uDestVar='SCRIPTINPUT', oFktNotify=self.On_InputRen)
        else:
    ...
    def GetSettingParFromVar2(self,*, uObjectName:str, uConfigName:str, uSettingParName:str) -> str:
        ...
        return GetVar(uVarName=f'CONFIG_{uSettingParName.upper()}', uContext=f'{uObjectName}/{uConfigName}')
