...
class cNotifications:
    ...
    def RegisterNotification(self,*, uNotification:str, fNotifyFunction:Callable, uDescription:str='', bQuiet:bool=False, aValueLinks:Optional[List[Dict]]=None, **kwargs) -> int:
        ...
        if iHash in self.dNotificationsHash:
            Logger.warning(f'Duplicate notification registration {uNotification}:{dArgs["notificationdescription"]}, replacing old one')
        ...
        if uFilterPageName:
            self.dFilterPageNames[f"{uNotification}_{uFilterPageName}"] = iHash
    ...
    def UnRegisterNotification_ByHash(self,*,iHash:int) -> None:
        ...
        if dEntry is not None:
            ...
            Logger.error('Tried to de-register notification with wrong hash')

    def SendNotification(self,*,uNotification:str, uDescription:str='', **kwargs) -> Dict:
        """
        ...
        for dFunction in aNotificationFunctions:
            ...
            Globals.oEvents.CopyActionPars(dTarget=dArgs, dSource=dFunction['args'], enReplaceOption=eReplaceOption.eDoNotReplaceTarget)
            if not dFunction['quiet']:
                Logger.debug(f'Sending notification {uNotification} {uDescription} to {dArgs["notificationdescription"]}')
