...
def SetVar(uVarName:str, oVarValue:Any, uContext:str='') -> None:
    """
    ...
    If you pass a dict, then for each dict member a varname with its value will get assigned (the dict member names will be separated by an underscore)
...
def SetVarWithOutVarTranslation(uVarName:str, oVarValue:Any, uContext:str='') -> None:
    """
    Sets a specific variable with a given value.
    ...
    if type(oVarValue) == dict:
        for uKey in oVarValue:
            _SetVarSub(uVarName = uRealVarName + '_' + uKey, oVarValue = oVarValue[uKey])
        return

    if type(oVarValue) == list or type(oVarValue) == tuple:
        i = 0
        for uValue in oVarValue:
            _SetVarSub(uVarName = uRealVarName + '_[' + ToUnicode(i)+']', oVarValue = oVarValue[uValue])
            i += 1
        return
    _SetVarSub(uVarName = uRealVarName, oVarValue = oVarValue)


def DelVar(uVarName:str, uContext:str='') -> None:
    """
...
def GetVar(uVarName:str, uContext:str='') -> str:
    """
    ...
    return ORCA.vars.Globals.dUserVars.get(uContext + ReplaceVars(uVarName),'')
...
def ExistVar(uVarName:str,  uContext:str='') -> bool:
    """
