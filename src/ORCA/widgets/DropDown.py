...
class cWidgetDropDown(cWidgetButton):
    ...
    def __init__(self,**kwargs):
        ...
        self.aCaptions        = []
        self.aSecondCaptions  = []
        self.uOrgCaptions     = u''
        self.aOrgActionNames  = []
        self.aActionNames     = []
    ...
    def InitWidgetFromXml(self,oXMLNode,oParentScreenPage, uAnchor):
        ...
        if bRet:
            self.tFrameColor    = GetColorFromHex(GetXMLTextAttribute(oXMLNode,u'framecolor',False,u'$var(dimmed)'))
            self.iFrameWidth    = GetXMLIntAttribute(oXMLNode,u'framewidth',  False,self.oDef.iDefMaxX/self.fRationX)*0.02
    ...
    def Create(self,oParent):
        ...
        try:
            ...
            if len(self.aActionNames)!=len(self.aCaptions) and len(self.aCaptions)>0:
                LogError(u'cWidgetDropDown: [%s] Captions do not match Actions: %s' % (self.uName,self.uActionName))
                bRet=False
            else:
    ...
    def UpdateWidgetSecondCaption(self):
        uCaption = ReplaceVars(self.uOrgSecondCaption)
        if uCaption == u'':
            return
