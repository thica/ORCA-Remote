...
class cWidgetScrollContainer(cWidgetPicture):
    ...
    The scrollcontainer widget is a container widget to place lines of wifget in a scrollable widget
    The scrollcontainer widget is based on the picture widget, please use the documentation for the common attributes

    The "CONTAINERVALUE" value is added to the action pars which refers to the widget text
    The "CONTAINERINDEX" index is added to the action pars which refers to the line order, eg: "0" is the first line in the list
    The "CONTAINERVARINDEX" index is added to the action pars which refers to the index number of the vararray
    ...
    |-
    |type
    |fixed: needs to be "SCOLLCONTAINER". Capital letters!
    |-
    |container
    ...
    def __init__(self,**kwargs):
        ...
        self.iNumRows:int                           = 0
        self.aContainerChilds:List[cWidgetBase]     = []
        self.aChilds:List[cWidgetBase]              = []


    def InitWidgetFromXml(self,oXMLNode:Element,oParentScreenPage:cScreenPage, uAnchor:str) -> bool:
        """ Reads further Widget attributes from a xml node """
        self.uRowHeightInit = GetXMLTextAttribute(oXMLNode=oXMLNode,uTag="rowheight",bMandantory=True,vDefault="%20")
    ...
    def DeleteScrollContainer(self):
        self.DeleteChilds()
    ...
    def CreateChilds(self,aChilds:List[cWidgetBase],aCaptions:List[List[Tuple[str,str]]]):
        ...
        for u in range(self.iNumRows):
            iIndex = 0
            uVarIndex = u'' # by purpose, we dont set it by child, we use the last known index, in case we have widgets without vars eg Buttons
            for oChild in aChilds:
