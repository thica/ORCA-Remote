...
class cScript(cDiscoverScriptTemplate):
    ...
    def CreateDiscoverList_ShowDetails(self,oButton:Button) -> None:
        ...
        uText:str = '$lvar(5029): %s \n'\
                    ...
                    '$lvar(5032): %s \n'\
                    '\n'\
                    '%s' % (dDevice.uFoundIP,dDevice.uFoundPort,dDevice.uFoundModel,dDevice.uFoundIdentifier,dDevice.uData)
    ...
    def Discover(self,**kwargs) -> Dict:
        ...
        try:
            ...
            self.ShowDebug(uMsg='No EISCP device found, possible timeout')
...
class cThread_Discover_EISCP(threading.Thread):
        ...
        try:
            ...
            if oSocket:
                ...
                while True:
                    ...
                    aReady:Tuple = select.select([oSocket], [], [],self.fTimeOut)
                    if aReady[0]:
                        # Get a response
                        ...
                        if dRet.bFound:
                            self.oCaller.ShowInfo(uMsg='Discovered device %s:%s at %s:' % (dRet.uFoundIdentifier, dRet.uFoundModel, dRet.uFoundIP))
                            cThread_Discover_EISCP.oWaitLock.acquire()
            ...
            self.oCaller.ShowError(uMsg='Error on discover EISCP (%s)' % self.uIPVersion, oException=e)
            if oSocket:
                oSocket.close()
            return
    ...
    def SendDiscover(self) -> Union[socket.socket,None]:
        if self.uIPVersion=='IPv4Only':
            ...
            oSocket.sendto(self.bOnkyoMagic, ('239.255.255.250', self.iPort))
            return oSocket
        ...
        if self.uIPVersion == 'IPv6Only':
            ...
            oSocket.sendto(self.bOnkyoMagic, ('ff02::c', self.iPort))
            return oSocket
