...
class cThread_Discover_UPNP(threading.Thread):
    ...
    def Discover(self) -> None:
        ...
        try:
            ...
            self.oCaller.ShowError(uMsg='Error on discover uPnP (%s)' % self.uIPVersion, oException=e)
            if oSocket:
                oSocket.close()
            return
    ...
    def SendDiscover(self):
        ...
        if self.uIPVersion=='IPv4Only':
            ...
            bMessage = ToBytes('M-SEARCH * HTTP/1.1\r\nHOST: %s:%d\r\nMAN: "ssdp:discover"\r\nMX: 2\r\nST: %s\r\n\r\n' % (uUDP_IP, iUDP_PORT, self.uST))
    ...
    def GetDeviceDetails(self,uData:str,tSenderAddr:Tuple) -> TypedQueryDict:
        ...
        oRet                     = TypedQueryDict()
        oRet.uFoundManufacturer  = ''
        oRet.uFoundModel         = ''
        oRet.uFoundFriendlyName  = ''
        oRet.uFoundServiceType   = ''
        ...
        if '200 OK' in oRet.sData:
            ...
            self.oCaller.ShowDebug(uMsg='Trying to get device details from  %s ->IPVersion: %s' % (uUrl,oRet.uIPVersion))
            ...
            try:
                ...
                if iStatusCode == 200 and "device" in self.oUrlRequest.result:
                    ...
                    try:
                        ...
                        pass
                    oRet.bFound              = True
                    self.oCaller.ShowDebug(uMsg='Found Device Manufacturer=%s Model=%s Friendlyname=%s IP=%s ST=%s' % (oRet.uFoundManufacturer, oRet.uFoundModel, oRet.uFoundFriendlyName, oRet.uFoundIP,oRet.uFoundServiceType))
            except Exception as e:
